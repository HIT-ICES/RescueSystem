...
def get_route_of_project(project_root):
    ...
    controller_dir = os.path.join(
        project_root, pkg_dir, pkg_name, "controller")
    if not os.path.exists(controller_dir):
        print(f'WARNING: Failed to find controller of {project_root}')
        return []
    ...
    for project_root in os.listdir("src"):
        ...
        for route in routes:
            if route in existing_routes:
                print(
                    f"Warning: found duplicate route {r} in project {project_root}")
